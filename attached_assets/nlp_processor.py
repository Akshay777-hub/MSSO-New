    ...
    try:
        ...
        for actor_name, scene_numbers in extracted_data['actor_scenes'].items():
            ...
            for scene_number in scene_numbers:
                ...
                actor_lines = len(re.findall(actor_pattern, scene_content))
                
                actor_scene = ActorScene(
                    actor_id=actor.id,
                    scene_id=scene.id,
                    lines_count=max(1, actor_lines)  # At least 1 line if they're in the scene
                )
        ...
        for constraint_data in extracted_data['constraints']:
            ...
            for constraint in constraint_data['constraints']:
                constraint_obj = SceneConstraint(
                    scene_id=scene.id,
        ...
        db.session.rollback()
        logging.error(f"Error saving screenplay data: {str(e)}")
        raise Exception(f"Failed to save screenplay data: {str(e)}")
